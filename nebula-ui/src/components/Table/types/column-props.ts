...
import { Align } from '@components/Table/types/common-props.ts'
import { VNode } from 'vue'
import { FilterMethods } from 'element-plus/es/components/table/src/table-column/defaults'
...
export type TableColumn = {
  ...
  renderHeader?: (data: { column: TableColumnCtx<any>; $index: number }) => VNode | string
...
/**
 * @description `element-plus` 的 `table` 中 `Table-column` 属性，已扩展
 */
export interface ColumnExtProp extends TableColumn {
  /** 是否隐藏 */
  hide?: boolean | CallableFunction
  /** 自定义列的内容插槽 */
  slot?: string
  render?: (scope: any) => VNode | string // 自定义单元格内容渲染（tsx语法）
  /** 自定义表头的内容插槽 */
  headerSlot?: string
  headerRender?: (scope: any) => VNode // 自定义表头内容渲染（tsx语法）
  /** 是否可编辑 */
  editable?: boolean
  /** 多级表头，内部实现原理：嵌套 `el-table-column` */
  children?: Array<ColumnExtProp>
}
