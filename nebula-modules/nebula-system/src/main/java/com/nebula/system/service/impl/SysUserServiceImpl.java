...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	public SysUserVO selectUserById(Long userId) {
		QueryWrapper queryWrapper = getQueryWrapper(SYS_USER.USER_ID.eq(userId), false);
		return sysUserMapper.selectOneByQueryAs(queryWrapper, SysUserVO.class);
	}

	@Override
	public SysUserVO selectUserByUserName(String username) {
		QueryWrapper queryWrapper = getQueryWrapper(SYS_USER.USER_NAME.eq(username), true);
		return sysUserMapper.selectOneByQueryAs(queryWrapper, SysUserVO.class);
	}

	private static QueryWrapper getQueryWrapper(QueryCondition queryCondition, Boolean isShowPassword) {
		QueryWrapper queryWrapper = QueryWrapper.create()
			.select(SYS_USER.USER_ID, SYS_USER.NICK_NAME, SYS_USER.USER_NAME, SYS_USER.EMAIL, SYS_USER.AVATAR,
				SYS_USER.PHONENUMBER, SYS_USER.SEX, SYS_USER.STATUS, SYS_USER.LOGIN_IP, SYS_USER.LOGIN_DATE,
				SYS_USER.DEPT_ID, SYS_DEPT.DEPT_NAME, SYS_DEPT.PARENT_ID, SYS_DEPT.ANCESTORS, SYS_ROLE.ROLE_ID,
				SYS_ROLE.ROLE_NAME, SYS_ROLE.ROLE_KEY, SYS_ROLE.DATA_SCOPE);
		if (isShowPassword) {
			queryWrapper.select(SYS_USER.PASSWORD);
		}
		return queryWrapper.from(SYS_USER.as("u"))
			.leftJoin(SYS_DEPT.as("d"))
			.on(SYS_DEPT.DEPT_ID.eq(SYS_USER.DEPT_ID))
			.leftJoin(SYS_USER_ROLE.as("sur"))
