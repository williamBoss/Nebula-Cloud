...
import { showMessage } from './status.ts' // 引入状态码文件
...
instance.interceptors.request.use(
  (config) => {
    ...
    if (token) {
      ...
      router.push(LOGIN_URL).catch((err) => {
        console.error(err)
      })
    }
  ...
  (error) => {
    showNotify({ type: 'danger', message: error.data.error.message })
...
instance.interceptors.response.use(
  ...
  (error) => {
    ...
    showNotify({ type: 'warning', message: '网络连接异常,请稍后再试!' })
  }
)
